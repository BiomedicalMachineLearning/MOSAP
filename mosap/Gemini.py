...
from napari_cosmx import MM_PER_PX, PX_PER_MM, ALPHA_MM_PER_PX, ALPHA_PX_PER_MM, PROTEIN_EXP_DIR, PROTEIN_EXP_ALT_DIR, \
    PROTEIN_EXP_ALT_ALT_DIR
...
class Gemini:
    ...
    def add_segmentation(self):
        ...
        layer = self.viewer.add_image(labels, contrast_limits=(0, 1), colormap="cyan",
                                      scale=(self.mm_per_px, self.mm_per_px), translate=self._top_left_mm(),
                                      blending="additive",
                                      rotate=self.rotate)
        layer.opacity = 0.5
    ...
    def __init__(self, path, viewer=None):
        ...
        if not self.is_protein:
            if not os.path.exists(os.path.join(self.folder, "targets.hdf5")):
                ...
                if self.alpha:
                    self.targets['global_x'] = self.targets.x - self.targets.X_mm * (1 / self.mm_per_px) - \
                                               self._top_left_mm()[1] * (1 / self.mm_per_px)
                    self.targets['global_y'] = self.targets.y + self.targets.Y_mm * (1 / self.mm_per_px) - \
                                               self._top_left_mm()[0] * (1 / self.mm_per_px)
                else:
                    self.targets['global_x'] = self.targets.x + self.targets.Y_mm * (1 / self.mm_per_px) - \
                                               self._top_left_mm()[1] * (1 / self.mm_per_px)
                    self.targets['global_y'] = self.targets.y - self.targets.X_mm * (1 / self.mm_per_px) - \
                                               self._top_left_mm()[0] * (1 / self.mm_per_px)
        else:
    ...
    def add_points(self, fov=None, color=None, gray_no_cell=None, point_size=1):
        ...
        face_color = cm.colors[1:][inv] if color is None else np.full(shape=(len(genes), 4),
                                                                      fill_value=transform_color(color))
        if gray_no_cell:
